...
Open `index.html` in any modern browser, or visit `/intro/` when the personal site is running. It has no build step, external dependency, or network requirement; it is intended to work from a local file or static host.
...
For an in-class presentation, open the deck in a browser at `/intro/` and use the browser’s normal full-screen mode if desired. Images are referenced from the existing site `images/` directory, so keep that relative structure when copying the deck elsewhere.
